...
const UNGROUPED_REQUESTS: SimpleEntry[] = [
  entry1,
  entry2bundle,
  entry3logoSvg,
  entry4logoSvg,
...
const GROUP2: GroupedEntry = {
  ...
  arrivalTime:
    new Date(entry17presentationAnalytics2.startedDateTime).getTime() +
    TIMESTAMP_100_MS,
...
const GROUP3: GroupedEntry = {
  ...
  arrivalTime:
    new Date(entry19faqBundle.startedDateTime).getTime() + TIMESTAMP_100_MS,
  root: entry18pageFaq,
  resources: [entry19faqBundle],
};

export const GROUPED_ENTRIES: GroupedEntry[] = [GROUP1, GROUP2, GROUP3];
