...
import { type Simulation } from "@src/interfaces/Entry";
...
describe("rebaseUrlIfNeeded", () => {
  it("should return same url", () => {
    ...
    ).toEqual("/blog/article-random.html");
  });
  it("should return url without baseUrl", () => {
    ...
    ).toEqual("https://fluflu.com/3615/test.php");
  });
});

const REBASED_GROUPED_ENTRIES = cloneDeep(GROUPED_ENTRIES).map((entries) => {
  entries.root.request.url = entries.root.request.url.includes(
    "https://pwn-association"
  )
    ? entries.root.request.url.slice(27)
    : entries.root.request.url;
  entries.resources = entries.resources.map((entry) => {
    entry.request.url = entry.request.url.includes("https://pwn-association")
      ? entry.request.url.slice(27)
      : entry.request.url;
    return entry;
  });
  return entries;
